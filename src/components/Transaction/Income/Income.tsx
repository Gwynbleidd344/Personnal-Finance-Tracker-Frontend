    ...
    return (
        <div className="z-50 flex h-[calc(96vh-120px)] w-full flex-col items-center rounded-lg bg-gray-100 lg:h-[96vh] dark:border-2 dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="flex min-h-full w-full flex-col rounded-2xl">
                <div className="flex flex-col border-gray-300 px-5 pt-5 pb-2.5 text-3xl font-bold md:flex-row md:items-center md:justify-between">
                    <div className="flex w-full flex-col justify-between border-b border-gray-300 md:flex-row md:items-center dark:border-gray-700">
                        <h3 className="mb-3 p-2 text-3xl font-bold text-gray-800 dark:text-gray-100">
                            {t('incomes', 'Incomes')}
                        </h3>
                        <div className="flex flex-row space-y-2 space-x-2 md:items-center md:space-y-0">
                            <button
                                onClick={() => {
                                    isModifying.current = {
                                        status: false,
                ...
                <AnimatePresence>
                    <motion.div
                        layout
                        className={`mt-2 w-full overflow-y-auto px-4 pt-3 ${view === 'grid' ? 'grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3' : 'flex flex-col space-y-0'}`}
                        style={{ maxHeight: 'calc(100vh - 220px)' }}
                    >
                        {view === 'list' && (
                            <div className="mx-5 mb-4 hidden grid-cols-12 items-center gap-3 rounded-sm border border-gray-300 px-4 py-2.5 font-semibold text-gray-600 md:grid md:pl-12 dark:border-gray-700 dark:text-gray-300">
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                        <motion.div
                            ...
                            className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg max-sm:mx-2"
                        >
                            ...
                            >
                                ...
                                <div className="mt-2 flex justify-end space-x-3">
                                    ...
                                    <button
                                        type="submit"
                                        className={`rounded-lg px-5 py-2 text-white ${isModifying.current.isDeleting ? 'bg-red-700 hover:bg-red-800' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                                    >
                                        {isModifying.current.status
                                            ? isModifying.current.isDeleting
                                                ? t('delete', 'Delete')
