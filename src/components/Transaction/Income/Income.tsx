...
type ActionsModel = { status: boolean; isDeleting: boolean; };

export default function Income() {
  const { t } = useTranslation();
  const [view, setView] = useState<'grid' | 'list'>(() => (localStorage.getItem('transactionView') as 'grid' | 'list') || 'grid');
  const toggleView = () => { const newView = view === 'grid' ? 'list' : 'grid'; setView(newView); localStorage.setItem('transactionView', newView); };
  const isModifying = useRef<ActionsModel>({ status: false, isDeleting: false });
  const cardIdRef = useRef<string>('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const openModal = () => setIsModalOpen(true);
  const closeModal = () => { isModifying.current = { status: false, isDeleting: false }; cardIdRef.current = ''; setIsModalOpen(false); };
  const [transactions, setTransactions] = useState<Transaction[]>([]);
  const token = getAccessToken();
  const [searchTerm, setSearchTerm] = useState('');
  const filteredTransactions = transactions.filter((tx) => {
    const matchesSearch = tx.name.toLowerCase().includes(searchTerm.toLowerCase()) || tx.category?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const fetchTransactions = async () => {
    if (!token) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/incomes`, { mode: 'cors', credentials: 'include', headers: { Authorization: token } });
  ...
  const handleAddTransaction = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/incomes`, {
        mode: 'cors', credentials: 'include', method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `${token}` },
        body: JSON.stringify({ amount, date, source, description: name }),
      });
  ...
  const handleUpdateTransaction = (e: React.FormEvent<HTMLFormElement>, incomeId?: string) => {
    e.preventDefault();
    ...
    const date = formData.get('date') as string;
    const source = formData.get('source') as string;
    fetch(`${import.meta.env.VITE_API_URL}/api/incomes/` + incomeId, { mode: 'cors', credentials: 'include', method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `${token}` }, body: JSON.stringify({ amount, date, source, description: name }) })
      .then(() => fetchTransactions())
      .catch(() => console.log('Error while updating'))
      .finally(() => { closeModal(); form.reset(); });
  };

  const handleDeleteTransaction = (e: React.FormEvent<HTMLFormElement>, incomeId?: string) => {
    e.preventDefault();
    incomeId = cardIdRef.current;
    fetch(`${import.meta.env.VITE_API_URL}/api/incomes/` + incomeId, { mode: 'cors', credentials: 'include', method: 'DELETE', headers: { 'Content-Type': 'application/json', Authorization: `${token}` } })
      .then(() => fetchTransactions())
      ...
      <div className="flex min-h-full w-full flex-col rounded-2xl">
        <div className="flex flex-col pb-2.5 border-gray-300 px-5 pt-5 text-3xl font-bold md:flex-row md:items-center md:justify-between">
          <div className="flex flex-col md:flex-row w-full md:items-center justify-between border-b border-gray-300 dark:border-gray-700">
            <h3 className="mb-3 p-2 text-3xl font-bold text-gray-800 dark:text-gray-100">{t('incomes','Incomes')}</h3>
            <div className="flex flex-row md:items-center space-y-2 md:space-y-0 space-x-2">
              <button onClick={() => { isModifying.current = { status:false, isDeleting:false }; cardIdRef.current=''; openModal(); }} className="flex cursor-pointer items-center gap-2 rounded-lg bg-gray-200 px-4 py-2 text-lg font-medium text-gray-800 shadow-sm transition-all duration-200 hover:bg-gray-300 hover:shadow-md active:scale-95">
                <FaPlus className="text-lg text-gray-600" /><span>{t('add','Add')}</span>
              </button>
              <div className="relative flex items-center">
                <SearchButton value={searchTerm} onChange={setSearchTerm} placeholder={t('search','Search')} />
              </div>
              <div className="hidden md:flex space-x-2">
                <button onClick={toggleView} className="flex h-12 w-12 items-center justify-center rounded-lg border border-gray-300 bg-gray-200 text-gray-800 transition hover:bg-gray-300 active:scale-95">{view === 'grid' ? <FaList /> : <FaThLarge />}</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence>
          <motion.div layout className={`mt-2 w-full overflow-y-auto px-4 pt-3 ${view === 'grid' ? 'grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3' : 'flex flex-col space-y-4'}`} style={{ maxHeight: 'calc(100vh - 220px)' }}>
            {filteredTransactions.map((t) => (
              <motion.div key={t.id} layout initial={{ scale:0, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.1, opacity:0 }} transition={{ duration:0.25 }} className="relative">
                <TransactionCard transaction={t} view={view} actions={{
                  onChange() { isModifying.current = { status:true, isDeleting:false }; cardIdRef.current = t.id; openModal(); },
                  onDelete() { isModifying.current = { status:true, isDeleting:true }; cardIdRef.current = t.id; openModal(); }
                }} />
              </motion.div>
            ))}
          </motion.div>
